...
- **Workspace Model**: Each customer has an independent local workspace (e.g., `customers/comm-it/`).
...
- **AWS Auth**: The tool does NOT manage AWS authentication. It relies on the user's environment (SSO, keys, etc.).
- **Client Factory**: All `boto3` sessions and clients MUST be created via `AwsClientFactory`. Service modules should not create their own sessions.
...
- **Dry Run**: Prefer `dry-run` behavior where possible; do not mutate AWS resources without clear command intent.
