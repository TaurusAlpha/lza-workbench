...
- **`lza import`**: Adopt existing local `aws-accelerator-config` without modifying customer files, validating layout and generating workspace metadata. Features full YAML syntax parsing with line/column diagnostics, version-aware official LZA schema validation (`global-config`, `organization-config`, `accounts-config` with mandatory accounts, `network-config`, `security-config`), remote/Git repository template provenance auto-detection (AWS CodeCommit, Git remotes, commits, branch, and tracked file counts), and automatic/`--repair` metadata restoration for partial or corrupted `lza-workspace.yaml` and `.lza/state.json` files.
...
- **`lza bootstrap`**: Create or validate AWS prerequisite resources required by LZA Workbench. Idempotently creates/validates the versioned, KMS-encrypted Workbench assets S3 bucket (`s3-lza-workbench-assets-<account-id>-<region>`) and the `lza-config-source` CodeCommit configuration repository (when `ConfigurationRepositoryLocation=codecommit`), with strict validation-only semantics on imported workspaces, presenting planned actions and confirmation, updating `lza-workspace.yaml` and `.lza/state.json`.
...
- **`lza pipeline start` & `lza pipeline watch`**: Reusable standalone CLI commands and workflows (`workflows/pipeline_start.py`, `workflows/pipeline_watch.py`) for triggering pipeline executions without config re-synchronization and monitoring active/historical executions with structured stage/action breakdown, direct CloudWatch Logs inspection, and automated root-cause failure diagnostics for failed CodeBuild/CloudFormation actions (e.g., TerminationProtection errors, stack validation failures).
