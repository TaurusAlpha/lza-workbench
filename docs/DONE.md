...
- **`lza config init`**: Authoritative workflow for creating and populating local `aws-accelerator-config` from packaged templates (`default`), with `--template`, `--force`, and `--dry-run` options. Implemented dynamic placeholder rendering engine (`configuration/rendering.py`) resolving `${path.to.field}` dot-notation paths (`customer.slug`, `lza.accelerator_prefix`, `aws.region`, `installer.options.management_account_email`, etc.) directly from `WorkspaceConfig` while preserving LZA-native `{{ ... }}` expressions, reporting unresolved placeholders, recording provenance in `lza-workspace.yaml`, and decoupling `lza init` to only create workspace metadata.
